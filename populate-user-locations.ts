...
async function populateUserLocations() {
  try {
    console.log('🌍 Starting location enrichment for maritime professionals...');
    ...
    console.log('Current statistics:', currentStats.rows[0]);
    ...
    for (let i = 0; i < usersWithoutLocation.rows.length; i++) {
      ...
      if (i % 10 === 0) {
        console.log(`📍 Processed ${i + 1}/${usersWithoutLocation.rows.length} users`);
      }
    }
