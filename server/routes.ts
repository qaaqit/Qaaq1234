...
Please provide only the improved prompt (15-20 words maximum) without any explanations or additional text:`;
  ...
  app.post("/api/semm/postcards/upload", sessionBridge, async (req, res) => {
    try {
      ...
      console.error('Error generating SEMM postcard upload URL:', error);
      res.status(500).json({ error: 'Failed to generate upload URL' });
    }
  });

  // Create a new SEMM postcard
  app.post("/api/semm/postcards", sessionBridge, async (req, res) => {
    try {
  ...
  // Get SEMM postcards for a specific SEMM code
  app.get("/api/semm/postcards/:semmCode/:semmType", async (req, res) => {
    try {
      const { semmCode, semmType } = req.params;
      const postcards = await storage.getSemmPostcards(semmCode, semmType);
      
      // Add user's like status if authenticated
      const userId = req.user?.id || req.user?.claims?.sub;
      if (userId) {
        for (const postcard of postcards) {
          const like = await storage.getSemmPostcardLike(userId, postcard.id);
          postcard.isLiked = !!like;
        }
      }

      res.json({ postcards });
    } catch (error) {
      console.error('Error fetching SEMM postcards:', error);
      res.status(500).json({ error: 'Failed to fetch postcards' });
    }
  });

  // Like a SEMM postcard
  app.post("/api/semm/postcards/:postcardId/like", sessionBridge, async (req, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      await storage.likeSemmPostcard(userId, postcardId);
      
      res.json({ success: true });
    } catch (error) {
      console.error('Error liking SEMM postcard:', error);
      res.status(500).json({ error: 'Failed to like postcard' });
    }
  });

  // Unlike a SEMM postcard
  app.delete("/api/semm/postcards/:postcardId/like", sessionBridge, async (req, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      const { postcardId } = req.params;
      await storage.unlikeSemmPostcard(userId, postcardId);
      
      res.json({ success: true });
    } catch (error) {
      console.error('Error unliking SEMM postcard:', error);
      res.status(500).json({ error: 'Failed to unlike postcard' });
    }
  });

  // Share a SEMM postcard
  app.post("/api/semm/postcards/:postcardId/share", sessionBridge, async (req, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      await storage.shareSemmPostcard(userId, postcardId, shareType);
      
      res.json({ success: true });
    } catch (error) {
      console.error('Error sharing SEMM postcard:', error);
      res.status(500).json({ error: 'Failed to share postcard' });
    }
  });

  // Delete a SEMM postcard
  app.delete("/api/semm/postcards/:postcardId", sessionBridge, async (req, res) => {
    try {
      const userId = req.user?.id || req.user?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      const { postcardId } = req.params;
      await storage.deleteSemmPostcard(postcardId, userId);
