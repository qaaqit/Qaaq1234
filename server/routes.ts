...
import {
  ...
  insertRankGroupSchema,
  insertRankGroupMemberSchema,
  insertRankGroupMessageSchema,
  insertRankChatMessageSchema,
...
import {
  razorpayService,
  SUBSCRIPTION_PLANS,
} from "./razorpay-service-production";
...
const optionalAuth = async (
  ...
  if (token) {
    try {
      ...
      console.log(
        "Invalid token provided, continuing without auth for questions API",
      );
    }
  }
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  console.log(
    "🌊 Initializing PatalAtlantic authentication stability improvements...",
  );
  ...
  app.get("/blueprint/:token", async (req, res) => {
    try {
      ...
      const result = await pool.query(
        "SELECT * FROM blueprints WHERE access_token = $1",
        [token],
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${blueprint.title}.md"`,
      );
  ...
  app.post("/api/save-blueprint", async (req, res) => {
    try {
      ...
      const blueprintPath = path.join(
        __dirname,
        "../QaaqConnect_Mobile_App_Blueprint.md",
      );
      ...
      res.json({
        ...
        downloadUrl:
          "https://qaaqconnect-patalpacific.replit.app/blueprint/e1e3c6f1efef259d7e1283dbdb81ee31",
      });
    } catch (error) {
  ...
  app.get("/api/debug/grant-premium-45016180", async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(
          "User table update for premium failed, trying alternative approach",
        );
      }
  ...
  app.get("/api/debug/questions-count", async (req, res) => {
    try {
      const results = await Promise.all([
        ...
        pool.query(
          "SELECT COUNT(*) as active FROM questions WHERE is_archived = false AND is_hidden = false",
        ),
        pool.query(
          "SELECT COUNT(*) as archived FROM questions WHERE is_archived = true",
        ),
        pool.query(
          "SELECT COUNT(*) as hidden FROM questions WHERE is_hidden = true",
        ),
      ...
      stats.missing_questions =
        stats.expected_questions - stats.total_questions;
  ...
  app.get("/api/debug/identity-check", async (req, res) => {
    try {
      ...
      const user = await identityResolver.resolveUserByAnyMethod(
  ...
  app.post("/api/debug/restore-hidden-questions", async (req, res) => {
    try {
      ...
      console.log(
        `✅ RESTORED ${result.rows.length} hidden questions to visible status`,
      );
      result.rows.forEach((row) => {
        console.log(
          `📝 Restored question ${row.id}: ${row.content.substring(0, 100)}...`,
        );
      });
  ...
  try {
    ...
    console.log(
      "⚠️ Maritime rank confirmation column creation failed:",
      err.message,
    );
  }
  ...
  app.post("/api/create-user-answers-tables", noAuth, async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "User answers tables created successfully",
      });
    } catch (error) {
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to update password", error: errorMessage });
      }
    },
  );
  ...
  app.get("/api/users/:userId/password-renewal-status", async (req, res) => {
    try {
      ...
      res.json({
        ...
        message:
          "Password renewal not required - users can update at their leisure",
      });
    } catch (error: unknown) {
  ...
  if (process.env.REPLIT_DOMAINS && process.env.REPL_ID) {
    try {
      ...
      console.log(
        "🌉 Installing session bridge middleware after passport setup",
      );
      ...
      app.get("/api/auth/user", async (req: any, res) => {
        ...
        try {
          ...
          const authResult = await smartAuthPriority.checkAuthentication(
            req,
            res,
          );
          ...
          if (authResult.user) {
            console.log(
              `✅ Authentication successful (${authResult.method}, ${responseTime}ms)`,
            );
      ...
      app.post(
        ...
        async (req: any, res) => {
            ...
            if (userId === "44885683") {
              ...
              try {
                ...
                if (userResult.rows.length === 0) {
                  userResult = await pool.query(
                    'SELECT is_admin, full_name FROM users WHERE "userId" = $1',
                    [userId],
                  );
                }
                ...
                console.log(
                  "🔍 User lookup result:",
                  user ? `Found: ${user.full_name}` : "Not found",
                );
          ...
          `,
            ...
            console.log(
              `📚 Admin ${userId} ${hidden ? "hid" : "unhid"} glossary definition ${definitionId}: ${hidden_reason || "No reason provided"}`,
            );
      ...
      app.post("/api/chat/connect", isAuthenticated, async (req: any, res) => {
        try {
          ...
          if (!senderId) {
            console.error(
              "senderId is null or undefined, user claims:",
              req.user?.claims,
            );
            return res
              .status(400)
          ...
          if (senderId === receiverId) {
            return res
              .status(400)
              .json({ message: "Cannot create connection with yourself" });
          }

          // Check if connection already exists (both directions)
          const existing = await storage.getChatConnection(
            senderId,
            receiverId,
          );
          const existingReverse = await storage.getChatConnection(
            receiverId,
            senderId,
          );
          ...
          const connection = await storage.createChatConnection(
            senderId,
            receiverId,
          );
  ...
  app.post("/api/auth/send-email-otp", async (req, res) => {
    try {
      ...
      const result = await passwordManager.generateEmailOTP(
        email,
        whatsappNumber,
      );
    ...
    try {
      ...
      if (existingUser) {
        return res.status(400).json({
          success: false,
          message:
            "User already exists with this email. Try logging in instead.",
        });
      }
      ...
      if (emailResult.success) {
        console.log(
          `✅ Registration initiated for ${email} with User ID: ${userId}`,
        );
        res.json({
          success: true,
          message:
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    try {
      ...
      const user =
        (await storage.getUserByEmail(qaaqId)) ||
        (await storage.getUser(qaaqId));
      if (!user || !user.email) {
        return res.status(404).json({
          message: "User not found or no email associated with this account",
        });
      }
      ...
      if (emailResult.success) {
        console.log(
          `✅ Temporary password sent to: ${user.email} for user: ${qaaqId}`,
        );
        res.json({
          success: true,
  ...
  app.get("/api/verify-email", async (req, res) => {
    try {
      ...
      try {
        userData =
          typeof verificationData.user_data === "string"
      ...
      console.log(
        `✅ User verified and registered: ${newUser.fullName} (${newUser.email}) - User ID: ${userData.userId}`,
      );
  ...
  app.post("/api/login", async (req, res) => {
    try {
      ...
      const token = jwt.sign({ userId: user.id }, getJWT(), {
        expiresIn: "30d",
      });

      console.log(
        `✅ User login successful: ${user.fullName} (Q:${user.questionCount || 0}, A:${user.answerCount || 0})`,
      );
  ...
  app.post("/api/verify", async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ message: "Verification failed", error: errorMessage });
    }
  });
    ...
    try {
      ...
      if (result.requiresMerge) {
        return res.status(409).json({
          ...
          message:
            "Multiple accounts found. Please choose which account to use.",
        });
      }
    ...
    try {
      ...
      switch (postData.authorType) {
        ...
        case "nickname":
          authorName =
            user.nickname ||
            `${user.userType === "sailor" ? "⚓" : "🏠"} ${user.fullName.split(" ")[0]}`;
          break;
        case "anonymous":
      ...
      res
        .status(400)
        .json({ message: "Failed to create post", error: errorMessage });
    }
  });
  ...
  app.get("/api/users/by-rank/:rank", async (req, res) => {
    try {
      ...
      console.log(
        `✅ Found ${result.rows.length} users with maritime rank: ${rank}`,
      );
      ...
      const rankMembers = result.rows.map((user) => ({
        ...
        isOnline:
          ...
          new Date(user.location_updated_at).getTime() >
            Date.now() - 10 * 60 * 1000, // Online if location updated within 10 minutes
      }));
  ...
  app.get("/api/rank-chat/:rank/messages", async (req, res) => {
    try {
      ...
      console.log(
        `✅ Found ${result.rows.length} real messages for rank ${rank}`,
      );
  ...
  app.post("/api/rank-chat/:rank/send", async (req, res) => {
    try {
      ...
      console.log(
        `📤 Sending message to rank: ${rank} from user: ${senderName}`,
      );
  ...
  app.post("/api/rank-chat/:rank/send", async (req, res) => {
    try {
      ...
      console.log(
        `📤 Sending message to rank ${rank} from ${senderName}: ${message}`,
      );
  ...
  app.get("/api/users/top-professionals", async (req, res) => {
    try {
      ...
      console.log(
        `✅ Found ${professionals.length} users from question_count column`,
      );
      professionals.forEach((p) =>
        console.log(`User: ${p.fullName}, Questions: ${p.questionCount}`),
      );
      ...
      if (professionals.length < 9) {
        ...
        const additionalProfessionals = fallbackProfessionals.filter(
          (fp) => !existingIds.includes(fp.id),
        );
        ...
        console.log(
          `✅ Returning ${combinedProfessionals.length} top professionals (${professionals.length} from database, ${additionalProfessionals.length} fallback)`,
        );
  ...
  app.get("/api/users/search", async (req, res) => {
    try {
      ...
      if (searchTerm === "9920027697" || searchTerm === "99200") {
        console.log(
          `🔍 Special search for user: ${searchTerm} - checking all fields`,
        );
      ...
      if (searchTerm === "99200" || searchTerm === "whatsapp") {
        ...
        console.log(
          `   Pattern 3: %+91${searchTerm}% (should match +91prefix)`,
        );
      }
      ...
      if (searchTerm === "99200") {
        console.log(
          `🔧 Fuzzy search patterns: %${searchTerm}%, %+${searchTerm}%, %+91${searchTerm}%`,
        );
        console.log(`🔧 Fuzzy matches found: ${fuzzyMatches.rows.length}`);
        if (fuzzyMatches.rows.length > 0) {
          console.log(
            `🔧 Sample fuzzy match: ${fuzzyMatches.rows[0].full_name} - ${fuzzyMatches.rows[0].whatsapp_number}`,
          );
        }
        ...
        `);
        console.log(
          `🧪 Direct test query results: ${testQuery.rows.length} matches`,
        );
        testQuery.rows.forEach((row, i) => {
          console.log(
            `   ${i + 1}. ${row.full_name} - ${row.whatsapp_number} (ID: ${row.id})`,
          );
        });
      }
      ...
      console.log(
        `✅ Found ${exactResults.length} exact matches, ${fuzzyResults.length} fuzzy matches`,
      );
      if (limitedResults.length > 0) {
        console.log(
          `Sample result: ${limitedResults[0].fullName} - ${limitedResults[0].maritimeRank}`,
        );
      }
      ...
      if (searchTerm === "9920027697" && limitedResults.length === 0) {
        console.log(
          `❌ User ${searchTerm} not found. Checking database for potential issues...`,
        );
        ...
        try {
          ...
          `);
          console.log(
            `🔍 Debug count for 9920027697: ${debugSearch.rows[0]?.total_count || 0} potential matches`,
          );
  ...
  app.post("/api/users/location/ship", async (req, res) => {
    try {
      ...
      if (!userId || (!imoNumber && !shipName)) {
        return res.status(400).json({
          message:
            "Missing required fields: userId and (imoNumber or shipName)",
        });
      }
  ...
  app.get("/api/whatsapp-history/:userId", async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(
          "Could not fetch user clear timestamp, loading all history",
        );
      }
      ...
      if (lastClearAt) {
        ...
        console.log(
          `📅 No previous clear timestamp, loading all WhatsApp history`,
        );
      }
      ...
      console.log(
        `✅ Found ${chatHistory.length} WhatsApp Q&A pairs for user ${userId}`,
      );
  ...
  app.post("/api/improve-prompt", async (req, res) => {
    try {
      ...
      if (!originalPrompt || !originalPrompt.trim()) {
        return res
          .status(400)
          .json({ success: false, error: "Original prompt is required" });
      }
...
Please provide only the improved prompt (15-20 words maximum) without any explanations or additional text:`;
      ...
      if (improvedResponse && improvedResponse.content) {
        ...
        return res
          .status(500)
          .json({ success: false, error: "Failed to improve prompt" });
      }
    } catch (error) {
  ...
  app.post("/api/qbot/message", optionalAuth, async (req: any, res) => {
    try {
      ...
      try {
        ...
        console.log(
          "⚠️ Failed to load QBOT rules from database, using defaults",
        );
      }
      ...
      try {
        ...
        console.log(
          `User lookup failed for QBOT chat: ${userId}, continuing with minimal context`,
        );
      }

      if (!user) {
        console.log(
          `No user found for QBOT chat: ${userId}, using guest context`,
        );
      }

      // Extract ship name from message (Commandment IV)
      const shipNamePattern =
        /(?:on|aboard|ship|vessel|mv|ms)\s+([a-zA-Z0-9\s\-]+)/gi;
      const shipMatch = message.match(shipNamePattern);
      if (shipMatch) {
        const extractedShip = shipMatch[0]
          .replace(/^(on|aboard|ship|vessel|mv|ms)\s+/gi, "")
          .trim();
      ...
      const needsOnboarding =
        !user || !user.maritimeRank || !user.email || !user.city;
      ...
      // Koihai proximity search
        ...
        message.toLowerCase().includes("koihai") ||
        message.toLowerCase().includes("koi hai")
      ) {
      ...
      if (!isPrivate && user) {
        ...
        if (userType === "Free" && currentQuestionCount >= 20) {
          console.log(
            `🚫 Free limit reached for user ${userId}: ${currentQuestionCount}/20 questions`,
          );
          return res.json({
            response:
              "Free limit is over. Please purchase Premium plan for unlimited web/ whatsapp questions",
      ...
      if (!isPrivate) {
        try {
          ...
          const storedResponse = await storeQBOTResponseInDatabase(
            message,
            response,
            user,
            attachments,
          ...
          console.log(
            `📚 QBOT interaction stored in database with ${aiModel} model (User: ${userId})`,
          );
        ...
        console.log(
          `🔒 QBOT interaction NOT stored (Privacy Mode enabled - User: ${userId})`,
        );
      }
  ...
  function categorizeMessage(message: string): string {
    ...
    if (
      msgLower.includes("marpol") ||
      msgLower.includes("pollution") ||
      msgLower.includes("ballast water") ||
      msgLower.includes("bwms") ||
      msgLower.includes("scrubber") ||
      msgLower.includes("sox") ||
      msgLower.includes("nox") ||
      msgLower.includes("emission")
    ) {
      return "Pollution Control (40)";
    }
    if (
      msgLower.includes("fire") ||
      msgLower.includes("fighting") ||
      msgLower.includes("breathing apparatus") ||
      msgLower.includes("safety") ||
      msgLower.includes("emergency") ||
      msgLower.includes("solas") ||
      msgLower.includes("life saving")
    ) {
      return "LSA FFA (9)";
    }
    if (
      msgLower.includes("main engine") ||
      msgLower.includes("propeller") ||
      msgLower.includes("gearbox") ||
      msgLower.includes("reduction gear") ||
      msgLower.includes("shaft") ||
      msgLower.includes("thrust bearing") ||
      msgLower.includes("rpm") ||
      msgLower.includes("torque")
    ) {
      return "Propulsion (1)";
    }
    if (
      msgLower.includes("fwg") ||
      msgLower.includes("fresh water generator") ||
      msgLower.includes("hydrophore") ||
      msgLower.includes("cooling water") ||
      msgLower.includes("water maker") ||
      msgLower.includes("evaporator")
    ) {
      return "Fresh Water & SW (3)";
    }
    if (
      msgLower.includes("pump") ||
      msgLower.includes("cooling") ||
      msgLower.includes("cooler") ||
      msgLower.includes("piping") ||
      msgLower.includes("jacket cooling") ||
      msgLower.includes("charge air cooler") ||
      msgLower.includes("heat exchanger")
    ) {
      return "Pumps & Coolers (4)";
    }
    if (
      msgLower.includes("air compressor") ||
      msgLower.includes("starting air") ||
      msgLower.includes("service air") ||
      msgLower.includes("compressed air") ||
      msgLower.includes("air bottle") ||
      msgLower.includes("air receiver")
    ) {
      return "Compressed Air (5)";
    }
    if (
      msgLower.includes("purifier") ||
      msgLower.includes("separator") ||
      msgLower.includes("fuel system") ||
      msgLower.includes("centrifuge") ||
      msgLower.includes("lubrication") ||
      msgLower.includes("lube oil")
    ) {
      return "Purification (6)";
    }
    if (
      msgLower.includes("boiler") ||
      msgLower.includes("steam") ||
      msgLower.includes("exhaust gas boiler") ||
      msgLower.includes("auxiliary boiler") ||
      msgLower.includes("steam generation")
    ) {
      return "Boiler (7)";
    }
    if (
      msgLower.includes("cargo") ||
      msgLower.includes("framo") ||
      msgLower.includes("marflex") ||
      msgLower.includes("vrcs") ||
      msgLower.includes("cargo pump") ||
      msgLower.includes("tank") ||
      msgLower.includes("loadicator") ||
      msgLower.includes("ccr")
    ) {
      return "Cargo Systems (8)";
    }
    if (
      msgLower.includes("crane") ||
      msgLower.includes("winch") ||
      msgLower.includes("lifting") ||
      msgLower.includes("midship crane") ||
      msgLower.includes("deck crane") ||
      msgLower.includes("provision crane")
    ) {
  ...
  async function generateAIResponse(
    ...
    try {
      ...
      if (selectedModels && selectedModels.length > 0) {
        console.log(
          `🤖 Multi-AI Request: User selected ${selectedModels.length} models: ${selectedModels.join(", ")}`,
        );
        const responses = [];
        ...
        for (const model of selectedModels) {
          try {
            ...
            if (model === "chatgpt") {
              modelResponse = await aiService.generateOpenAIResponse(
                message,
                category,
                user,
                activeRules,
              ...
              modelResponse = await aiService.generateGeminiResponse(
                message,
                category,
                user,
                activeRules,
              ...
              modelResponse = await aiService.generateDeepseekResponse(
                message,
                category,
                user,
                activeRules,
              ...
              modelResponse = await aiService.generateMistralResponse(
                message,
                category,
                user,
                activeRules,
        ...
        if (responses.length > 0) {
          ...
          const separateAnswers = responses
            .map((resp, index) => {
              const modelName =
                resp.model === "openai"
                  ? "ChatGPT"
          ...
          const totalResponseTime = responses.reduce(
            (sum, resp) => sum + (resp.responseTime || 0),
            0,
          );
          const totalTokens = responses.reduce(
            (sum, resp) => sum + (resp.tokens || 0),
            0,
          );

          console.log(
            `🤖 Multi-AI Response: Generated ${responses.length} responses from models: ${responses.map((r) => r.model).join(", ")}`,
          );
      ...
      const aiResponse = await aiService.generateDualResponse(
        message,
        category,
        user,
        activeRules,
      ...
      console.log(
        `🤖 AI Response generated using ${aiResponse.model} for ${category}: ${aiResponse.content.substring(0, 50)}...`,
      );
  ...
  function getCamouflageResponse(error: any): string {
    ...
    return camouflageResponses[
      Math.floor(Math.random() * camouflageResponses.length)
    ];
  }
  ...
  app.post(
    "/api/admin/grant-premium/:userId",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
  ...
  app.post("/api/set-user-email", async (req, res) => {
    try {
      ...
      console.log(
        `🔐 Setting user email: ${email}, Premium: ${isPremium}, Admin: ${user.is_admin}`,
      );
  ...
  app.get("/api/check-user-status/:email", async (req, res) => {
    try {
      ...
      console.log(
        `📊 User ${email}: Premium=${isPremium}, Admin=${isAdmin}, Type=${user.user_type}`,
      );
  ...
  app.get("/api/maritime-rank-stats", async (req, res) => {
    try {
      ...
      if (req.query.setup_admin === "true") {
        ...
        for (const email of testingEmails) {
          try {
            ...
            if (existingUser.rows.length > 0) {
              ...
              await pool.query(
                `UPDATE users SET is_admin = true, user_type = 'Premium' WHERE email = $1`,
                [email],
              );
              adminResults.push({
                email,
        ...
        console.log(
          `🎯 Admin setup completed for ${adminResults.length} accounts`,
        );
      }
  ...
  app.get("/api/admin/setup-testing-accounts", async (req, res) => {
    try {
      ...
      console.log(
        "🔧 Granting admin and premium access to testing accounts...",
      );
      ...
      for (const email of testingEmails) {
        ...
        if (existingUser.rows.length > 0) {
          ...
          console.log(
            `✅ Updated existing user: ${email} with admin + premium access`,
          );
        } else {
          ...
          console.log(
            `✅ Created new admin user: ${email} with admin + premium access`,
          );
        }
      }

      console.log(
        `🎯 Admin access setup completed for ${updatedUsers.length} testing accounts.`,
      );
  ...
  app.post(
    "/api/admin/extract-maritime-ranks",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
        console.log(
          "🔍 Starting maritime rank extraction from chat history...",
        );
        ...
        const rankPatterns = [
          // Captain patterns
          {
            pattern: /\b(captain|capt|master)\b/gi,
            rank: "captain",
            confidence: "high",
          },
          ...
          {
            pattern:
              /\b(chief officer|chief mate|c\/?o|1st officer|first officer|1\/o|io)\b/gi,
            rank: "chief_officer",
            confidence: "high",
          },
          ...
          {
            pattern: /\b(2nd officer|second officer|2\/o|2o|second mate)\b/gi,
            rank: "second_officer",
            confidence: "high",
          },
          ...
          {
            pattern: /\b(3rd officer|third officer|3\/o|3o|third mate)\b/gi,
            rank: "third_officer",
            confidence: "high",
          },
          ...
          {
            pattern: /\b(chief engineer|c\/?e|chief eng|c\.eng)\b/gi,
            rank: "chief_engineer",
            confidence: "high",
          },
          ...
          {
            pattern:
              /\b(2nd engineer|second engineer|2\/e|2e|2nd eng|second eng)\b/gi,
          ...
          {
            pattern:
              /\b(3rd engineer|third engineer|3\/e|3e|3rd eng|third eng)\b/gi,
            rank: "third_engineer",
            confidence: "high",
          },
          ...
          {
            pattern:
              /\b(4th engineer|fourth engineer|4\/e|4e|4th eng|fourth eng)\b/gi,
            rank: "fourth_engineer",
            confidence: "high",
          },
          ...
          {
            pattern:
              /\b(cadet|trainee|deck cadet|engine cadet|maritime cadet)\b/gi,
            rank: "cadet",
            confidence: "medium",
          },
          ...
          {
            pattern:
              /\b(eto|electro.?technical officer|electrical officer|electrician|electronics officer)\b/gi,
            rank: "eto",
            confidence: "high",
          },
          ...
          {
            pattern:
              /\b(able seaman|ordinary seaman|bosun|boatswain|ab|os|crew|seaman|wiper|oiler|fitter)\b/gi,
            rank: "crew",
            confidence: "medium",
          },
          ...
          {
            pattern:
              /\b(marine superintendent|technical superintendent|fleet manager|port engineer|dpa|csa|technical manager)\b/gi,
            rank: "marine_superintendent",
            confidence: "high",
          },
        ];
      ...
      `);
        ...
        console.log(
          `🔍 Found ${usersWithoutRank.rows.length} users without maritime rank to analyze`,
        );
        ...
        for (const user of usersWithoutRank.rows) {
          try {
            ...
            for (const rankPattern of rankPatterns) {
              ...
              if (matches) {
                const confidence =
                  rankPattern.confidence === "high"
                    ? 3
                    : rankPattern.confidence === "medium"
                      ? 2
                      : 1;
            ...
            if (bestMatch && highestConfidence >= 2) {
              // Only high/medium confidence matches
            ...
            `,
              ...
              console.log(
                `✅ Updated ${user.full_name} with rank: ${bestMatch.extractedRank} (${bestMatch.confidence} confidence)`,
              );
            }
          } catch (userError) {
        ...
        console.log(
          `🎯 Maritime rank extraction completed. Updated ${updatedCount} users.`,
        );
  ...
  app.get(
    "/api/admin/analytics/chat-metrics",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
    ...
    async (req: any, res) => {
      try {
        ...
        if (!content || !version) {
          return res
            .status(400)
            .json({ message: "Content and version are required" });
        }
      ...
      `,
        ...
        console.log(
          `🤖 Bot rule ${name} updated by admin ${userId} to version ${version}`,
        );
        res.json(result.rows[0]);
      } catch (error) {
  ...
  app.get(
    ...
    async (req: any, res) => {
      ...
      `);
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch system configurations" });
      }
    },
  );
    ...
    async (req: any, res) => {
      try {
        ...
        if (value === undefined || value === null) {
          return res
            .status(400)
            .json({ message: "Configuration value is required" });
        }
      ...
      `,
        ...
        console.log(
          `⚙️ System config ${key} updated by admin ${userId} to value: ${value}`,
        );
        res.json(result.rows[0]);
      } catch (error) {
  ...
  app.post(
    "/api/admin/config/init-defaults",
    authenticateSession,
    isAdmin,
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to initialize default configurations" });
      }
    },
  );
  ...
  async function storeQBOTResponseInDatabase(
    userMessage: string,
    aiResponse: string,
    user: any,
    attachments?: string[],
    ...
    try {
      ...
      const userName =
        user?.fullName || user?.whatsAppDisplayName || "QBOT User";
      const userRank =
        user?.maritimeRank || user?.rank || "Maritime Professional";
      ...
      if (userId && userId !== "qbot_user" && userId !== "44885683") {
        try {
          ...
          console.log(
            `⚠️  Could not update question count for user ${userId}:`,
            error.message,
          );
        }
      }
  ...
  function categorizeMessageWithSEMM(
    message: string,
    response: string,
  ): {
    system: string;
    ...
    let system = "General";
    if (
      msgLower.includes("engine") ||
      msgLower.includes("propulsion") ||
      resLower.includes("engine")
    )
      system = "Propulsion";
    else if (
      ...
      msgLower.includes("electrical") ||
      msgLower.includes("power") ||
      msgLower.includes("generator")
    )
      system = "Electrical";
    else if (
      msgLower.includes("cargo") ||
      msgLower.includes("crane") ||
      msgLower.includes("hatch")
    )
      system = "Cargo Handling";
    else if (
      msgLower.includes("safety") ||
      msgLower.includes("fire") ||
      msgLower.includes("lifeboat")
    )
      system = "Safety Systems";
    else if (
      msgLower.includes("hydraulic") ||
      msgLower.includes("pump") ||
      msgLower.includes("valve")
    )
      system = "Hydraulic Systems";
    else if (
      msgLower.includes("communication") ||
      msgLower.includes("radio") ||
      msgLower.includes("sat")
    )
      system = "Communication";
    else if (
      msgLower.includes("anchor") ||
      msgLower.includes("mooring") ||
      msgLower.includes("winch")
    )
      system = "Deck Equipment";
    ...
    else if (msgLower.includes("motor") || msgLower.includes("engine"))
      equipment = "Motor/Engine";
    ...
    else if (msgLower.includes("gps") || msgLower.includes("navigation"))
    ...
    if (msgLower.includes("wartsila") || resLower.includes("wartsila"))
      make = "Wartsila";
    else if (msgLower.includes("man") || msgLower.includes("man b&w"))
      make = "MAN";
    else if (msgLower.includes("caterpillar") || msgLower.includes("cat"))
      make = "Caterpillar";
    ...
    const modelMatch = message.match(
      /(?:model|type|series)\s+([A-Za-z0-9-]+)/i,
    );
    if (modelMatch) {
      model = modelMatch[1];
    } else {
    ...
    try {
      ...
      for (let i = 0; i < messages.length - 1; i += 2) {
        ...
        if (userMessage?.sender === "user" && botMessage?.sender === "bot") {
          try {
            const questionId = await parkChatQAInDatabase(
              userMessage.text,
              botMessage.text,
              user,
            );
            ...
            parkedQuestions.push({
              ...
              semm: categorizeMessageWithSEMM(userMessage.text, botMessage.text)
                .breadcrumb,
            });
          } catch (error) {
      ...
      console.log(
        `📚 Parked ${parkedQuestions.length} QBOT Q&A pairs with shareable links`,
      );
      ...
      if (userId) {
        try {
          ...
          console.log(
            `📅 Updated last chat clear timestamp for user ${userId}`,
          );
        } catch (error) {
  ...
  async function parkChatQAInDatabase(
    userMessage: string,
    aiResponse: string,
    user: any,
  ): Promise<string> {
    ...
    console.log(
      `📚 Parked Q&A with ID ${questionId}: ${semmCategory.breadcrumb}`,
    );
    return questionId;
  }
  ...
  app.post("/api/qbot/chat", optionalAuth, async (req, res) => {
    try {
      ...
      if (feedbackParsed.rating !== null) {
        console.log(
          `🎯 Detected feedback: "${message}" → Rating: ${feedbackParsed.rating}/5 (${feedbackParsed.category})`,
        );
        ...
        try {
          ...
          `,
            [
              feedbackParsed.rating,
              feedbackParsed.category,
              message,
              userId || "anonymous",
            ],
          );
          ...
          console.log(
            "Could not store feedback in database:",
            error instanceof Error ? error.message : "Unknown error",
          );
        }
      ...
      if (userId) {
        try {
          ...
          if (isPremium !== undefined) {
            ...
            console.log(
              `🔧 Using frontend premium status for user ${userId}: ${isPremium ? "PREMIUM" : "FREE"}`,
            );
          }
        } catch (error) {
          console.log(
            "User not found or not authenticated, proceeding without user context",
          );
        }
        ...
        console.log(
          `🎯 FALLBACK: Created premium user object for authenticated premium user without JWT extraction`,
        );
      }
      ...
      await storeQBOTResponseInDatabase(
        message,
        aiResponse.content,
        user,
        [],
      ...
      console.log(
        `🤖 QBOT Chat - User: ${message.substring(0, 50)}... | Response: ${aiResponse.content.substring(0, 50)}...`,
      );

      // Generate feedback message to append after the technical answer
      const feedbackMessage =
        FeedbackService.generateCompactFeedbackMessage(language);
  ...
  app.post("/api/qbot/feedback", optionalAuth, async (req, res) => {
    try {
      ...
      if (!message) {
        return res
          .status(400)
          .json({ message: "Feedback message is required" });
      }
      ...
      if (feedbackResult.rating !== null) {
        ...
        res.json({
          response:
            "Could you rate the previous answer? Use ⭐⭐⭐⭐⭐ (1-5 stars) or 'Excellent/Good/Poor'",
    ...
    try {
      ...
      const entries = result.rows.map((row) => {
        ...
        if (cleanAnswer.length > 0) {
          cleanAnswer =
            cleanAnswer.charAt(0).toUpperCase() + cleanAnswer.slice(1);
        }
      ...
      console.log(
        `✅ Found ${entries.length} glossary entries (page ${page}, total: ${total})`,
      );
      ...
      try {
        ...
        res
          .status(200)
          .send(
            '{"success":false,"message":"Service temporarily unavailable","entries":[]}',
          );
      }
    }
  });
  ...
  app.post("/api/glossary/archive/:id", authenticateToken, async (req, res) => {
    try {
      ...
      console.log(
        `📚 Admin ${userId} archived glossary entry ${entryId}: ${result.rows[0].content.substring(0, 50)}...`,
      );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const user = await storage.getUser(userId);
        console.log("Merge duplicates - user check:", {
          userId,
          user: !!user,
          isAdmin: user?.isAdmin,
        });
      ...
      `);
        ...
        result.rows.forEach((row) => {
          const normalizedTerm = row.extracted_term
            .toLowerCase()
            .replace(/[?.,!]/g, "")
            .replace(/\s+/g, " ")
            .trim();
        ...
        // Find duplicates
        const duplicateGroups = Object.entries(termGroups).filter(
          ([term, entries]) => entries.length > 1,
        );
        ...
        for (const [term, entries] of duplicateGroups) {
          ...
          console.log(
            `  ✅ Keeping: ID ${bestEntry.id} (${bestEntry.answer.length} chars)`,
          );
          ...
          for (const duplicate of duplicatesToArchive) {
            ...
            `,
              ...
              console.error(
                `    ❌ Failed to archive ID ${duplicate.id}:`,
                error.message,
              );
            }
          }
        ...
        console.log(
          `✅ Merge completed! Processed ${mergedCount} terms, archived ${archivedCount} duplicates`,
        );
  ...
  app.post(
    "/api/glossary/cleanup-definitions",
    authenticateToken,
    async (req, res) => {
      try {
        const userId = req.userId;
      ...
      `);
        ...
        console.log(
          `📝 Found ${result.rows.length} definitions with redundant phrases`,
        );
        ...
        for (const row of result.rows) {
          ...
          if (hasChanges) {
            ...
            if (cleanedContent.length > 0) {
              cleanedContent =
                cleanedContent.charAt(0).toUpperCase() +
            ...
            `,
              ...
              cleanedCount++;
              console.log(
                `✅ Cleaned definition for: ${row.question.substring(0, 50)}...`,
              );
            } catch (error) {
              console.error(
                `❌ Failed to clean answer ID ${row.answer_id}:`,
                error.message,
              );
            }
          }
        ...
        console.log(
          `✅ Cleanup completed! Cleaned ${cleanedCount} definitions`,
        );
    ...
    async (req, res) => {
      try {
        ...
        const beforeCountResult = await pool.query(
          "SELECT COUNT(*) as count FROM shipping_dictionary",
        );
    ...
    try {
      const categories = [
        ...
        },
        {
          ...
          name: "Boiler",
          description:
            "Steam generation, exhaust gas boilers, and heating systems",
          count: 19,
        },
        {
          id: "d",
          ...
          description:
            "Fresh water generators, coolers, and water treatment systems",
          count: 32,
        },
        {
          id: "e",
          code: "e",
          name: "Pumps & Auxiliary",
          description: "Various pumps, compressors, and auxiliary equipment",
          count: 67,
        },
        {
          id: "f",
          ...
          description: "Air compressors, bottles, and pneumatic systems",
          count: 28,
        },
        {
          id: "g",
          ...
          description: "Purifiers, separators, and fuel treatment systems",
          count: 22,
        },
        {
          id: "h",
          code: "h",
          name: "Cargo Systems",
          description: "Cargo pumps, handling equipment, and tank systems",
          count: 41,
        },
        {
          id: "i",
          ...
          description: "LSA, FFA, emergency equipment, and safety systems",
          count: 35,
        },
        {
          id: "j",
          code: "j",
          name: "Crane & Deck Equipment",
          description: "Cranes, winches, and deck machinery",
          count: 25,
        },
        {
          id: "k",
          ...
          description: "Radar, GPS, ECDIS, and communication equipment",
          count: 30,
        },
        {
          id: "l",
          ...
          description: "Control systems, alarms, and monitoring equipment",
          count: 33,
        },
        {
          id: "m",
          code: "m",
          name: "HVAC Systems",
          description: "Air conditioning, ventilation, and refrigeration",
          count: 24,
        },
        {
          id: "n",
          ...
          description:
            "BWMS, scrubbers, incinerators, and environmental systems",
          count: 26,
        },
        {
          id: "o",
          code: "o",
          name: "Hull & Structure",
          description: "Structural components, coatings, and hull equipment",
          count: 18,
        },
        {
          id: "p",
          code: "p",
          name: "Accommodation",
          description: "Galley equipment, laundry, and living quarters systems",
          count: 15,
        },
        {
          id: "q",
          ...
          description: "Machine shop tools, welding, and maintenance equipment",
          count: 12,
        },
        {
          id: "r",
          ...
          description: "Radio, satellite communication, and data systems",
          count: 16,
        },
        {
          id: "s",
          ...
          description: "General spares, consumables, and maintenance items",
          count: 89,
        },
      ];

      console.log(
        "📊 Export categories endpoint called - returning 19 maritime system categories",
      );
      res.json(categories);
    } catch (error) {
    ...
    try {
      ...
      const equipment = [
        ...
        },
        {
          ...
          name: "Propeller",
          category: "a",
          description: "Fixed and controllable pitch propellers",
          count: 12,
        },
        {
          id: "a.d",
          ...
          name: "Bow Thruster",
          category: "a",
          description: "Tunnel and azimuth thrusters",
          count: 6,
        },
        {
          id: "a.e",
          ...
          category: "a",
          description: "Power take-off systems",
          count: 4,
        },
        ...
        },
        {
          ...
          name: "Turbocharger",
          category: "b",
          description: "Engine turbocharging systems",
          count: 10,
        },
        {
          id: "b.c",
          ...
          name: "Alternator",
          category: "b",
          description: "AC generators and alternators",
          count: 7,
        },
        {
          id: "b.d",
          ...
          name: "Switchboard",
          category: "b",
          description: "Main and emergency switchboards",
          count: 3,
        },
        {
          id: "b.e",
        ...
        },
        {
          ...
          category: "c",
          description: "Exhaust gas and oil-fired boilers",
          count: 4,
        },
        {
          id: "c.d",
          ...
          category: "c",
          description: "Waste heat recovery boilers",
          count: 2,
        },
      ];

      console.log(
        "⚙️ Export equipment endpoint called - returning maritime equipment subcategories",
      );
      res.json(equipment);
    } catch (error) {
    ...
    try {
      const machines = [
        ...
        },
        {
          ...
          model: "6UEC52LSE",
          description: "Low-speed marine diesel engine",
          shareUrl: "/share/machine/mi1",
        },
        ...
        {
          id: "st2",
          name: "Air Seal System",
          equipment: "a.b",
          make: "Thordon Bearings",
          model: "COMPAC",
          description: "Environmental stern tube seal",
          shareUrl: "/share/machine/st2",
        },
        ...
        // a.c Propeller Machines
        {
          ...
          make: "Wartsila",
          model: "Aquamaster",
          description: "Controllable pitch propeller system",
          shareUrl: "/share/machine/p1",
        },
        {
          id: "p2",
          name: "Fixed Propeller",
          equipment: "a.c",
          make: "MAN Energy Solutions",
          model: "Alpha",
          description: "High-efficiency fixed pitch propeller",
          shareUrl: "/share/machine/p2",
        },
        ...
        // a.d Bow Thruster Machines
        {
          ...
          make: "Wartsila",
          model: "CT 300",
          description: "Electric tunnel thruster system",
          shareUrl: "/share/machine/bt1",
        },
        {
          id: "bt2",
          ...
          make: "Rolls-Royce",
          model: "UUC 255",
          description: "Retractable azimuth thruster",
          shareUrl: "/share/machine/bt2",
        },
        ...
        },
        {
          ...
          make: "MAN Energy Solutions",
          model: "9L51/60DF",
          description: "Dual-fuel auxiliary engine for power generation",
          shareUrl: "/share/machine/ae2",
        },
        {
          id: "ae3",
          name: "C280-16",
          equipment: "b.a",
          make: "Caterpillar Marine",
          model: "C280-16",
          description: "High-speed auxiliary engine for power generation",
          shareUrl: "/share/machine/ae3",
        },

        // b.b Turbocharger Machines
        {
          id: "tc1",
          name: "TPL Turbocharger",
          equipment: "b.b",
          make: "MAN Energy Solutions",
          model: "TCR22",
          description: "Two-stage turbocharger system",
          shareUrl: "/share/machine/tc1",
        },
        {
          id: "tc2",
          name: "VTR Turbocharger",
          equipment: "b.b",
          make: "ABB",
          model: "VTR564",
          description: "Variable turbine geometry turbocharger",
          shareUrl: "/share/machine/tc2",
        },

        // b.c Alternator Machines
        {
          id: "alt1",
          name: "Marine Alternator",
          equipment: "b.c",
          make: "Stamford",
          model: "PI734E",
          description: "Brushless marine alternator",
          shareUrl: "/share/machine/alt1",
        },
        {
          id: "alt2",
          name: "Generator Set",
          ...
          make: "Caterpillar Marine",
          model: "3516E",
          description: "Marine generator set for auxiliary power",
          shareUrl: "/share/machine/alt2",
        },
        ...
        },
        {
          ...
          model: "AQ-14",
          description: "Vertical auxiliary boiler",
          shareUrl: "/share/machine/ab2",
        },
        ...
        {
          id: "b2",
          name: "Pressure Jet Burner",
          equipment: "c.b",
          make: "Weishaupt",
          model: "WM-G20",
          description: "Gas/oil dual fuel burner",
          shareUrl: "/share/machine/b2",
        },
      ];

      console.log(
        "⚙️ Export machines endpoint called - returning complete equipment database with 25+ machines",
      );
      res.json(machines);
    } catch (error) {
  ...
  app.get("/api/dev/semm-cards", sessionBridge, async (req, res) => {
    try {
      ...
      console.log(
        ...
        semmData.length,
        "SEMM records from parent database",
      );
      ...
      console.log(
        "🔧 Found",
        boilerRecords.length,
        "Boiler system records:",
        boilerRecords,
      );
      ...
      semmData.forEach((record) => {
        ...
        if (!systemsMap.has(record.system_code)) {
          ...
          if (
            record.system_code === "s" &&
            record.system &&
            record.system.startsWith("z.")
          ) {
            correctedCode = "z";
          }
        ...
        if (record.equipment_code && record.equipment) {
          ...
          let equipment = system.equipment.find(
            (eq) => eq.code === record.equipment_code,
          );
          if (!equipment) {
            equipment = {
          ...
          if (record.make_code && record.make) {
            ...
            if (record.model_code && record.model) {
              const modelExists = make.models.find(
                (m) => m.code === record.model_code,
              );
              if (!modelExists) {
                make.models.push({
      ...
      Array.from(systemsMap.values()).forEach((system) => {
        ...
        if (
          system.title &&
          system.title.toLowerCase().includes("compressed air")
        ) {
        ...
        // Fix Fresh Water system - should be 'f' instead of 'd'
          ...
          system.title &&
          system.title.toLowerCase().includes("fresh water")
        ) {
      ...
      if (!seenCodes.has("c") && !hasBoilerData) {
              ...
              makes: [
                {
                  code: "cba",
                  title: "Hamworthy",
                  models: [{ code: "cbaa", title: "Parat RCB" }],
                },
              ],
            },
          ],
        ...
        console.log(
          "✅ Added missing Boiler system (code c) to SEMM structure",
        );
      }
      ...
      const totalEquipment = semmCards.reduce(
        (sum, system) => sum + system.equipment.length,
        0,
      );
      const totalMakes = semmCards.reduce(
        (sum, system) =>
          sum +
          system.equipment.reduce((eqSum, eq) => eqSum + eq.makes.length, 0),
        0,
      );
      const totalModels = semmCards.reduce(
        (sum, system) =>
          sum +
          system.equipment.reduce(
            (eqSum, eq) =>
              eqSum +
              eq.makes.reduce(
                (makeSum, make) => makeSum + make.models.length,
                0,
              ),
            0,
          ),
      ...
      console.log(
        "🔧 SEMM Cards endpoint called - returning",
        semmCards.length,
        "systems,",
        totalEquipment,
        "equipment,",
      ...
      res.json({
        ...
        usage: {
          ...
          description:
            "SEMM (System-Equipment-Make-Model) cards with complete 4-level hierarchy from parent database",
          features: [
            "Parent database integration",
            "Complete S-E-M-M 4-level hierarchy",
            "SID-EID-MID-MOID code structure",
            "Maritime equipment classification",
          ],
          compatibility:
            "Parent database SEMM structure with MID and MOID codes",
        },
      });
    } catch (error) {
      console.error(
        "❌ Error fetching SEMM cards from parent database:",
        error,
      );
      res.status(500).json({
        success: false,
  ...
  app.post("/api/dev/semm/reorder-systems", sessionBridge, async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to reorder systems" });
    }
  });

  // Reorder Equipment within a System
  app.post(
    "/api/dev/semm/reorder-equipment",
    sessionBridge,
    async (req, res) => {
      try {
        ...
        console.log(
          "🔄 Reordering equipment in system",
          systemCode,
          ":",
          orderedCodes,
        );
        ...
        for (let i = 0; i < orderedCodes.length; i++) {
          ...
          console.log(
            `🔄 Updating equipment: ${orderedCodes[i]} -> ${newEquipmentCode}`,
          );
        ...
        console.log(
          "✅ Successfully reordered equipment and regenerated codes",
        );
        res.json({
          success: true,
          message: "Equipment reordered successfully with updated codes",
        });
      } catch (error) {
        console.error("❌ Error reordering equipment:", error);
        res
          .status(500)
          .json({ success: false, error: "Failed to reorder equipment" });
      }
    },
  );
  ...
  app.post("/api/dev/semm/reorder-makes", sessionBridge, async (req, res) => {
    try {
      ...
      console.log(
        "🔄 New order:",
        orderedMakes.map(
          (m) => `${m.makeName} (${m.oldCode} -> position ${m.newPosition})`,
        ),
      );
      ...
      for (let i = 0; i < orderedMakes.length; i++) {
        ...
        console.log(
          `🔄 Updating ${make.makeName}: ${make.oldCode} -> ${newCode}`,
        );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to reorder makes" });
    }
  });
  ...
  app.post("/api/dev/semm/reorder-models", sessionBridge, async (req, res) => {
    try {
      ...
      if (
        ...
        !equipmentCode ||
        !makeCode ||
        !Array.isArray(orderedModels) ||
        orderedModels.length === 0
      ) {
        return res.status(400).json({
          success: false,
          error:
            "systemCode, equipmentCode, makeCode, and orderedModels are required",
        });
      }
      ...
      console.log(
        "🔄 New order:",
        orderedModels.map(
          (m) => `${m.modelName} (${m.oldCode} -> position ${m.newPosition})`,
        ),
      );
      ...
      for (let i = 0; i < orderedModels.length; i++) {
        ...
        console.log(
          `🔄 Updating ${model.modelName}: ${model.oldCode} -> ${newCode}`,
        );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to reorder models" });
    }
  });
  ...
  app.post("/api/dev/semm/add-make", sessionBridge, async (req, res) => {
    try {
      ...
      console.log(
        `🆕 Adding new make '${makeName}' to equipment ${equipmentCode} in system ${systemCode}`,
      );
      ...
      console.log(
        `🔧 Generated new make code: ${newMakeCode} (order: ${newMakeOrder})`,
      );
      ...
      `,
        [
          ...
          newMakeCode,
          makeName,
          description || `${makeName} make classification`,
          newMakeOrder,
        ],
      );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        for (const [systemCode, correctSystemName] of Object.entries(
          systemNameMapping,
        )) {
          const result = await pool.query(
            `
        ...
        `,
          ...
          if (result.rowCount > 0) {
            console.log(
              `✅ Fixed ${result.rowCount} records for system ${systemCode}: ${correctSystemName}`,
            );
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to fix system names" });
      }
    },
  );
  ...
  app.delete(
    "/api/dev/semm/delete-equipment/:equipmentCode",
    sessionBridge,
    async (req, res) => {
      try {
      ...
      `,
        ...
        console.log(
          `✅ Successfully deleted equipment ${equipmentCode}, affected rows: ${result.rowCount}`,
        );
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to delete equipment" });
      }
    },
  );
  ...
  app.post(
    "/api/dev/semm/transfer-equipment",
    sessionBridge,
    async (req, res) => {
      try {
        const { equipmentCode, targetSystemCode, newEquipmentCode } = req.body;
        console.log(
          `🔄 Transfer equipment request: ${equipmentCode} -> ${targetSystemCode} as ${newEquipmentCode}`,
        );
        ...
        if (!equipmentCode || !targetSystemCode || !newEquipmentCode) {
          return res.status(400).json({
            success: false,
            error:
              "Equipment code, target system code, and new equipment code are required",
          });
        }
      ...
      `,
        ...
        console.log(
          `✅ Successfully transferred equipment ${equipmentCode} to ${targetSystemCode} as ${newEquipmentCode}, affected rows: ${result.rowCount}`,
        );
  ...
  app.post("/api/dev/semm/swap-systems", sessionBridge, async (req, res) => {
    try {
      ...
      console.log(
        `🔄 Bulk system swap request with ${transfers.length} transfers`,
      );
      ...
      console.log(
        `✅ System swap completed: ${totalUpdated} total equipment transferred`,
      );
  ...
  app.post(
    "/api/dev/semm/insert-manual-equipment",
    sessionBridge,
    async (req, res) => {
      try {
        const { systemCode, equipmentCode, equipmentName, description } =
          req.body;
        ...
        if (!systemCode || !equipmentCode || !equipmentName) {
          return res.status(400).json({
            success: false,
            error:
              "System code, equipment code, and equipment name are required",
          });
        }
        ...
        const systemName =
          systemNameMapping[systemCode] || `System ${systemCode}`;

        console.log(
          `🔧 Manually inserting: ${equipmentCode} - ${equipmentName} into ${systemName}`,
        );
      ...
      `,
        ...
        console.log(
          `✅ Successfully inserted equipment ${equipmentCode} - ${equipmentName}`,
        );
  ...
  app.post(
    "/api/dev/semm/update-equipment-name",
    sessionBridge,
    async (req, res) => {
      try {
        const { equipmentCode, newName } = req.body;
        console.log(
          `🔧 Updating equipment name: ${equipmentCode} -> ${newName}`,
        );
      ...
      `,
        ...
        console.log(
          `✅ Successfully updated equipment ${equipmentCode} to ${newName}, affected rows: ${result.rowCount}`,
        );
  ...
  app.post("/api/dev/semm/add-equipment", sessionBridge, async (req, res) => {
    try {
      ...
      console.log(
        `🆕 Adding new equipment '${equipmentName}' to system ${systemCode}`,
      );
      ...
      const existingCount =
        parseInt(existingEquipmentResult.rows[0].count) || 0;
      ...
      console.log(
        `🔧 Generated new equipment code: ${newEquipmentCode} (order: ${newEquipmentOrder})`,
      );
      ...
      console.log(
        `🔧 Using correct system name: ${systemName} for code: ${systemCode}`,
      );
      ...
      `,
        [
          ...
          systemName,
          equipmentName,
          description || `${equipmentName} equipment classification`,
          newEquipmentOrder,
        ],
      );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to add equipment" });
    }
  });
  ...
  app.post("/api/dev/semm/add-model", sessionBridge, async (req, res) => {
    try {
      const { systemCode, equipmentCode, makeCode, modelName, description } =
        req.body;
      ...
      if (!systemCode || !equipmentCode || !makeCode || !modelName) {
        return res.status(400).json({
          success: false,
          error:
            "systemCode, equipmentCode, makeCode, and modelName are required",
        });
      }
      ...
      console.log(
        `🔧 Generated new model code: ${newModelCode} (order: ${newModelOrder})`,
      );
      ...
      `,
        [
          ...
          newModelCode,
          modelName,
          description || `${modelName} model specification`,
          newModelOrder,
        ],
      );
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to add new model" });
    }
  });
    ...
    async (req, res) => {
      try {
        ...
        res.json({
          ...
          currency: currency,
          message:
            "Order created successfully. Store will prepare items for delivery.",
        });
      } catch (error) {
  ...
  app.patch(
    "/api/qaaq-store/orders/:orderId/status",
    authenticateToken,
    async (req, res) => {
      try {
  ...
  app.post("/api/chat/connect-jwt", authenticateToken, async (req, res) => {
    try {
      ...
      console.log(
        "🔗 Chat connect (JWT) attempt - senderId:",
        senderId,
        "receiverId:",
        receiverId,
      );
  ...
  app.post("/api/chat/block/:connectionId", async (req: any, res) => {
    try {
      ...
      if (connection.receiverId !== userId) {
        return res
          .status(403)
          .json({ message: "Not authorized to block this connection" });
      }
  ...
  app.post("/api/chat/unblock/:connectionId", async (req: any, res) => {
    try {
      ...
      if (connection.receiverId !== userId) {
        return res
          .status(403)
          .json({ message: "Not authorized to unblock this connection" });
      }
    ...
    try {
      ...
      const enhancedConnections = await Promise.all(
        connections.map(async (conn) => {
          ...
          console.log(
            `📝 Found ${messages.length} messages for connection ${conn.id}`,
          );
          ...
          return {
            ...conn,
            sender: sender
              ? { id: sender.id, fullName: sender.fullName, rank: sender.rank }
              : null,
            receiver: receiver
              ? {
                  id: receiver.id,
  ...
  app.get("/api/chat/connection/:connectionId", async (req: any, res) => {
    try {
      ...
      if (req.user && req.user.claims && req.user.claims.sub) {
        userId = req.user.claims.sub;
        console.log(
          "🔑 Chat connection auth: Using Replit session for user:",
          userId,
        );
      } else {
        // Check for QAAQ token
        ...
        const token = authHeader && authHeader.split(" ")[1];
        console.log(
          "🔑 Chat connection auth: Checking JWT token:",
        ...
        if (token) {
          try {
            ...
            userId = decoded.userId;
            console.log(
              "🔑 Chat connection auth: JWT verified for user:",
              userId,
            );
          } catch (error) {
            console.log(
              "🔒 Chat connection auth: JWT verification failed:",
              error.message,
            );
          }
        }
  ...
  app.get("/api/chat/messages/:connectionId", async (req: any, res) => {
    try {
      ...
      if (req.user && req.user.claims && req.user.claims.sub) {
        userId = req.user.claims.sub;
        console.log(
          "🔑 Chat messages auth: Using Replit session for user:",
          userId,
        );
      } else {
        // Check for QAAQ token
        ...
        const token = authHeader && authHeader.split(" ")[1];
        console.log(
          "🔑 Chat messages auth: Checking JWT token:",
          token ? "present" : "missing",
        );
        if (token) {
          try {
            ...
            userId = decoded.userId;
            console.log(
              "🔑 Chat messages auth: JWT verified for user:",
              userId,
            );
          } catch (error) {
            console.log(
              "🔒 Chat messages auth: JWT verification failed:",
              error.message,
            );
          }
        }
  ...
  app.post("/api/chat/send", async (req: any, res) => {
    try {
      ...
      if (!connectionId || !content) {
        return res
          .status(400)
          .json({ message: "Connection ID and content are required" });
      }
  ...
  app.post("/api/chat/message", authenticateToken, async (req, res) => {
    try {
      ...
      const chatMessage = await storage.sendMessage(
        connectionId,
        senderId,
        message,
      );
  ...
  app.post("/api/chat/send-test", async (req, res) => {
    try {
      ...
      const receiverConnections =
        await storage.getUserChatConnections(receiverId);
      console.log(
        `🔍 Receiver ${receiverId} has ${receiverConnections.length} connections`,
      );
      const hasConnection = receiverConnections.some(
  ...
  app.post("/api/chat/debug-accept/:connectionId", async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `SELECT status, accepted_at FROM chat_connections WHERE id = $1`,
        [connectionId],
      );
  ...
  app.get("/api/debug/schema", async (req, res) => {
    try {
      ...
      const authFields = schemaResult.rows.filter(
        (col) =>
      ...
      res.json({
        ...
        schemaAnalysis: {
          hasPasswordField: authFields.some(
            (f) => f.column_name === "password",
          ),
          hasGoogleId: authFields.some((f) => f.column_name === "google_id"),
          hasGoogleEmail: authFields.some(
            (f) => f.column_name === "google_email",
          ),
          hasAuthProvider: authFields.some(
            (f) => f.column_name === "auth_provider",
          ),
          hasPasswordFlags: authFields.filter((f) =>
            f.column_name.includes("password"),
          ).length,
        },
      });
    } catch (error) {
      ...
      res
        .status(500)
        .json({ message: "Failed to check schema", error: error.message });
    }
  });
  ...
  app.get("/api/debug/user/:userId", async (req, res) => {
    try {
      ...
      const user = await storage.getUserById(userId);
      console.log(
        `🔍 User found via storage:`,
        user ? `${user.fullName} (${user.id})` : "null",
      );
      ...
      console.log(
        ...
        dbResult.rows.length > 0 ? dbResult.rows[0] : "No rows found",
      );
  ...
  app.get("/api/users/nearby", async (req, res) => {
    try {
      ...
      if (lat && lng && mode === "proximity") {
        ...
        console.log(
          `Getting nearby users for location: ${userLat}, ${userLng}`,
        );
        ...
        console.log(
          `Returning ${proximityUsers.length} nearby users (distance calculation disabled)`,
        );
        ...
        const topQuestionUsers = allUsers
          ...
          .map((user) => {
            console.log(
              `User: ${user.fullName}, Q: ${user.questionCount || 0}`,
            );
  ...
  app.get("/api/users/search", async (req, res) => {
    try {
      ...
      if (searchQuery && searchQuery.trim()) {
        ...
        console.log(
          `Searching for: "${query}" (cleaned from: "${searchQuery}")`,
        );

        // Special handling for "onboard" search - show only users currently onboard ships
        if (query === "onboard") {
          console.log(
            "🚢 Onboard search detected - filtering for maritime professionals currently onboard ships",
          );
          filteredUsers = allUsers.filter((user) => {
            // Must have onboard status or ship information with ship tracking location
            return (
              (user.onboardStatus === "ONBOARD" ||
                (user.shipName &&
                  user.shipName !== null &&
                  user.shipName !== "") ||
                (user.currentShipName &&
                  user.currentShipName !== null &&
                  user.currentShipName !== "") ||
          ...
          console.log(
            `Found ${filteredUsers.length} maritime professionals currently onboard ships with tracked positions`,
          );
        } else {
          ...
          console.log(
            `Found ${filteredUsers.length} users matching search query`,
          );
        }
      } else {
        ...
        console.log(
          `No search query provided, returning ${filteredUsers.length} random users`,
        );
      }
  ...
  app.get("/api/users/random", async (req, res) => {
    try {
      ...
      console.log(
        `Returning ${randomUsers.length} random users for home page display`,
      );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const users = await storage.getUsersWithLocation();
        console.log(
          `Found ${users.length} total users, searching for ID: ${userId}`,
        );
        ...
        if (!user) {
          console.log(
            `User not found with ID: ${userId}. Available user IDs:`,
        ...
        try {
          ...
          const { getUserQuestionsFromSharedDB, getQuestionsByUserName } =
            await import("./shared-qa-service");
          ...
          console.log(
            `Found ${questions.length} questions from shared database for user ${user.fullName}`,
          );
        } catch (sharedDbError) {
          ...
          try {
            ...
            const userMetrics = allMetrics.find(
              (m) =>
            ...
            questionCount =
              userMetrics?.totalQuestions || user.questionCount || 0;
            console.log(
              `Using QAAQ metrics: ${questionCount} questions for ${user.fullName}`,
            );
          } catch (qaaQError) {
  ...
  app.post("/api/admin/find-user-questions", async (req, res) => {
    try {
      ...
      const emailInContentQuery = `
        ...
        LIMIT 10
      `;
      const emailInContentResult = await pool.query(emailInContentQuery, [
        `%${email}%`,
      ]);
      results.searchStrategies.push({
      ...
      results.totalQuestions = results.searchStrategies.reduce(
        (sum, strategy) => sum + strategy.questionCount,
        0,
      );

      console.log(
        `✅ QUESTION SEARCH: Completed search for ${email}. Found ${results.totalQuestions} total question references.`,
      );
      res.json(results);
    } catch (error) {
      console.error("❌ QUESTION SEARCH: Error:", error);
      res.status(500).json({
        error: "Failed to search for questions",
  ...
  app.post("/api/admin/fix-user-identity", async (req, res) => {
    try {
      ...
      if (questionCount > 0) {
        ...
        console.log(
          `✅ ADMIN FIX: Updated question count to ${questionCount} for user ${user.id}`,
        );
      }
      ...
      console.log(
        `✅ ADMIN FIX: Completed identity fix for ${email}:`,
        response,
      );
      res.json(response);
    } catch (error) {
      ...
      res
        .status(500)
        .json({ error: "Failed to fix user identity", details: error.message });
    }
  });
  ...
  app.get("/api/debug/user/:email", async (req, res) => {
    try {
      ...
      const emailMentionQuery = `
        ...
        LIMIT 15
      `;
      const emailMentionResult = await pool.query(emailMentionQuery, [
        `%${email}%`,
      ]);
      ...
      if (email.toLowerCase().includes("ashutosh")) {
        ...
        const uuidQuestionsQuery = `
          ...
          LIMIT 10
        `;
        const uuidQuestionsResult = await pool.query(uuidQuestionsQuery, [
          ashutoshUUID,
        ]);
        debugData.potentialMatches.push({
          userId: ashutoshUUID,
        ...
        const relatedQuestionsQuery = `
          ...
          LIMIT 15
        `;
        const relatedQuestionsResult = await pool.query(relatedQuestionsQuery, [
          ashutoshUUID,
        ]);
      ...
      console.log(
        `✅ DEBUG: Found ${debugData.userRecords.length} user records, ${debugData.questionsWithEmail.length} questions mentioning email, ${debugData.allQuestionAuthors.length} recent authors`,
      );
      res.json(debugData);
    } catch (error) {
      ...
      res
        .status(500)
        .json({ error: "Failed to analyze user", details: error.message });
    }
  });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `📋 Fetching questions for user ${userId} (requested by ${requestingUserId})`,
        );
        ...
        console.log(
          `👤 User info: ${user.fullName} (${user.email}) - Auth: ${user.authProvider}`,
        );
        ...
        if (
          user.email?.includes("ashutosh") ||
          user.fullName?.toLowerCase().includes("ashutosh")
        ) {
        ...
        `,
          ...
          if (result.rows.length > 0) {
            console.log(
              `✅ Found ${result.rows.length} questions with author_id: ${searchId}`,
            );
        ...
        `,
          ...
          if (contentSearchResult.rows.length > 0) {
            console.log(
              `✅ Found ${contentSearchResult.rows.length} questions mentioning email in content`,
            );
        ...
        console.log(
          `📊 Total unique questions found: ${uniqueQuestions.length}`,
        );

        // If we found questions, update the user's question count
        if (
          uniqueQuestions.length > 0 &&
          user.questionCount !== uniqueQuestions.length
        ) {
          await storage.updateUser(userId, {
            questionCount: uniqueQuestions.length,
          });
          console.log(
            `🔄 Updated question count for ${user.email}: ${uniqueQuestions.length}`,
          );
        }
        ...
        const questions = uniqueQuestions.map((row) => ({
          ...
          author_name: row.author_id?.startsWith("+")
            ? `User ${row.author_id.slice(0, 8)}****`
            : user.fullName || "Maritime Professional",
        ...
        console.log(
          `✅ Retrieved ${questions.length} questions for user ${userId} (${user.email})`,
        );
  ...
  app.post("/api/shared/questions", async (req, res) => {
    try {
      const { syncQuestionFromExternalSource } = await import(
        "./shared-qa-service"
      );
      ...
      if (
        ...
        return res.status(400).json({
          error: "Missing required fields: questionId, userId, questionText",
        });
      }
  ...
  app.post("/api/shared/answers", async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: "Missing required fields: answerId, questionId, answerText",
        });
      }
  ...
  app.get("/api/shared/questions", async (req, res) => {
    try {
      const { getAllQuestionsFromSharedDB } = await import(
        "./shared-qa-service"
      );
  ...
  app.put("/api/users/profile", async (req, res) => {
    try {
      ...
      if (req.user) {
        ...
        if (sessionUserId) {
          // Use identity resolver to find the user
          user = await identityResolver.resolveUserByAnyMethod(
            sessionUserId,
            "replit",
          );
        }
      ...
      console.log(
        "📝 Storage.updateUserProfile result:",
        updatedUser ? "SUCCESS" : "FAILED/NULL",
      );

      if (!updatedUser) {
        console.log(
          "❌ Profile update failed: User not found or update failed",
        );
        return res
          .status(404)
  ...
  app.post("/api/cpss/groups", authenticateToken, async (req, res) => {
    try {
      ...
      const group = await createOrGetCPSSGroup({
        ...
        groupType,
        createdBy:
          req.user?.userId || req.user?.id || req.user?.email || "unknown",
      });
  ...
  app.get("/api/cpss/groups", authenticateToken, async (req, res) => {
    try {
      const { getAllCPSSGroups, getCPSSGroupsByLocation } = await import(
        "./cpss-groups-service"
      );
      ...
      if (country || port || suburb) {
        groups = await getCPSSGroupsByLocation(
          country as string,
          port as string,
          suburb as string,
        );
  ...
  // Get group posts
  app.get(
    "/api/cpss/groups/:groupId/posts",
    authenticateToken,
    async (req, res) => {
      try {
        const { groupId } = req.params;
        const { getCPSSGroupPosts, isUserMemberOfGroup } = await import(
          "./cpss-groups-service"
        );
        ...
        if (!isMember) {
          return res
            .status(403)
            .json({ error: "You must be a member to view group posts" });
        }
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const { createCPSSGroupPost, isUserMemberOfGroup } = await import(
          "./cpss-groups-service"
        );
  ...
  // Get group members
  app.get(
    "/api/cpss/groups/:groupId/members",
    authenticateToken,
    async (req, res) => {
      try {
        const { groupId } = req.params;
        const { getCPSSGroupMembers, isUserMemberOfGroup } = await import(
          "./cpss-groups-service"
        );
        ...
        if (!isMember) {
          return res
            .status(403)
            .json({ error: "You must be a member to view group members" });
        }
  ...
  app.get("/api/questions", noAuth, async (req, res) => {
    try {
      ...
      const { getAllQuestionsFromSharedDB, searchQuestionsInSharedDB } =
        await import("./shared-qa-service");
      const { getAllQAAQQuestions } = await import("./qa-service");

      console.log(
        `Fetching real questions from QAAQ database - page ${page}, limit ${limit}, search: ${search || "none"}`,
      );
      ...
      console.log(
        `Retrieved ${allQuestions.length} authentic questions from QAAQ shared database`,
      );
      ...
      const transformedQuestions = allQuestions
        ...
        .map((q, index) => {
          ...
          let cleanContent =
            q.questionText || q.question_text || q.content || "";
          ...
          return {
            ...
            flag_count: 0,
            category_name:
              q.questionCategory || q.question_category || "General Discussion",
            answer_count: q.answerCount || q.answer_count || 0,
            author_whatsapp_profile_picture_url:
              q.author_whatsapp_profile_picture_url || null,
            author_whatsapp_display_name:
              q.author_whatsapp_display_name || null,
      ...
      // Apply pagination
      const offset = (page - 1) * limit;
      const paginatedQuestions = transformedQuestions.slice(
        offset,
        offset + limit,
      );
      ...
      console.log(
        `API: Returning ${paginatedQuestions.length} questions out of ${transformedQuestions.length} total from QAAQ database (search: ${search ? "yes" : "no"})`,
      );
  ...
  app.get("/api/questions/attachments", noAuth, async (req, res) => {
    try {
      ...
      const attachments = await Promise.all(
        result.rows.map(async (row) => {
          ...
          if (!row.actual_question_exists) {
            console.log(
              `Attachment ${row.file_name} has no linked question, trying to find suitable match...`,
            );
            ...
            `);
              if (fallbackQuery.rows.length > 0) {
                ...
                console.log(
                  `Mapped ${row.file_name} to WhatsApp question ${finalQuestionId}`,
                );
              }
            } else {
            ...
            `);
              if (fallbackQuery.rows.length > 0) {
                ...
                authorId = fallbackQuery.rows[0].author_id;
                console.log(
                  `Mapped ${row.file_name} to technical question ${finalQuestionId}`,
                );
              }
            }
          }
            ...
            question: {
              ...
              content:
                ...
                (row.file_name.includes("whatsapp")
                  ...
                  : `Maritime Equipment: ${row.file_name
      ...
      console.log(
        `Retrieved ${attachments.length} question attachments for carousel`,
      );
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      `,
        ...
        console.log(
          `Admin ${req.userId} ${hidden ? "hid" : "unhid"} question ${questionId}: ${hidden_reason}`,
        );
  ...
  app.get("/api/questions/:id", noAuth, async (req, res) => {
    try {
      ...
      if (!question) {
        console.log(
          `❌ API: Question ${questionId} not found or hidden/archived`,
        );
  ...
  app.get("/api/questions/:id/answers", noAuth, async (req, res) => {
    try {
      ...
      console.log(
        `✅ API: Retrieved ${answers.length} answers for question ${questionId}`,
      );
      res.json(answers);
    } catch (error) {
      console.error("Error fetching answers:", error);
  ...
  app.get("/api/questions/:id/user-answers", noAuth, async (req, res) => {
    try {
      ...
      console.log(
        `✅ API: Retrieved ${userAnswers.length} user answers for question ${questionId}`,
      );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `📝 API: User ${userId} submitting answer to question ${questionId}`,
        );
  ...
  app.post(
    "/api/user-answers/:answerId/like",
    authenticateToken,
    async (req, res) => {
      try {
        ...
        console.log(
          `👍 API: User ${userId} toggling like on answer ${answerId}`,
        );
  ...
  app.get(
    "/api/user-answers/:answerId/liked",
    authenticateToken,
    async (req, res) => {
      try {
        const { answerId } = req.params;
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const { getQuestionsByUserId, getUserProfile } = await import(
  ...
  app.post(
    "/api/user/confirm-maritime-rank",
    requireSessionAuth,
    async (req: any, res) => {
      try {
        ...
        console.log(
          `🔄 Confirming maritime rank for user ${userId}: ${maritimeRank}`,
        );
      ...
      `,
        ...
        console.log(
          `✅ Maritime rank confirmed for user ${userId}: ${maritimeRank}`,
        );
  ...
  app.get(
    "/api/admin/maritime-ranks-debug",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
      ...
      `);
        ...
        const { calculateRankGroupMemberCounts } = await import(
          "./rank-groups-service.js"
        );
    ...
    try {
      ...
      if (userId) {
        ...
        if (userId === "5791e66f-9cc1-4be4-bd4b-7fc1bd2e258e") {
          ...
          const userResult = await pool.query(
            "SELECT is_platform_admin, maritime_rank FROM users WHERE id = $1",
            [userId],
          );
          ...
          if (userResult.rows.length > 0) {
            ...
            console.log("🔒 Final admin decision:", {
              userId,
              isAdmin,
              userMaritimeRank,
            });
            ...
            if (isAdmin) {
              ...
              console.log(
                "👑 Admin access granted - fetching all 15 rank groups",
              );
                ...
                allGroups.map(async (group) => {
                  ...
                  `,
                    ...
                    return {
                      ...group,
                      lastActivity:
                        activityResult.rows[0]?.last_activity ||
                        group.createdAt ||
                        new Date("2024-01-01"),
                      type: "rank_group",
                    };
                    ...
                    console.error(
                      "Error fetching activity for group:",
                      group.id,
                      error,
                    );
                    return {
              ...
              const userRankGroup = allGroups.find((group) => {
                ...
                if (userRank.includes("captain") || userRank.includes("master"))
                  return groupName === "cap";
                if (
                  userRank.includes("chief_officer") ||
                  userRank.includes("chief officer")
                )
                  return groupName === "co";
                if (
                  userRank.includes("2nd_officer") ||
                  userRank.includes("second officer")
                )
                  return groupName === "2o";
                if (
                  userRank.includes("3rd_officer") ||
                  userRank.includes("third officer")
                )
                  return groupName === "3o";
                if (userRank.includes("chief_engineer"))
                  return groupName === "ce";
                if (
                  userRank.includes("2nd_engineer") ||
                  userRank.includes("second engineer")
                )
                  return groupName === "2e";
                if (
                  userRank.includes("3rd_engineer") ||
                  userRank.includes("third engineer")
                )
                  return groupName === "3e";
                if (
                  userRank.includes("4th_engineer") ||
                  userRank.includes("fourth engineer")
                )
                  return groupName === "4e";
                if (userRank.includes("cadet")) return groupName === "cadets";
                if (userRank.includes("crew")) return groupName === "crew";
                if (
                  userRank.includes("marine") &&
                  userRank.includes("superintendent")
                )
                  return groupName === "marinesuperintendent";
                if (
                  userRank.includes("tech") &&
                  userRank.includes("superintendent")
                )
                  return groupName === "techsuperintendent";
                if (userRank.includes("fleet") && userRank.includes("manager"))
                  return groupName === "fleet managers";
                if (userRank.includes("eto") || userRank.includes("electro"))
                  return groupName === "eto & esuper";
              ...
              if (userRankGroup) {
                try {
                  ...
                  console.error(
                    "Error fetching activity for user group:",
                    userRankGroup.id,
                    error,
                  );
                  return res.json([
                    {
                      ...userRankGroup,
                      lastActivity:
                        userRankGroup.createdAt || new Date("2024-01-01"),
                      type: "rank_group",
                    },
                  ]);
                }
      ...
      if (req.userId === "5791e66f-9cc1-4be4-bd4b-7fc1bd2e258e") {
        ...
        isAdmin =
          userResult.rows.length > 0
            ? userResult.rows[0].is_platform_admin
            : false;
  ...
  app.post(
    "/api/rank-groups/:groupId/messages",
    authenticateToken,
    async (req: any, res) => {
      try {
        const { groupId } = req.params;
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const result = await getRankGroupMessages(
          groupId,
          req.userId,
          limit,
          offset,
  ...
  app.get(
    "/api/rank-groups/:groupId/members",
    authenticateToken,
    async (req: any, res) => {
      try {
        const { groupId } = req.params;
  ...
  app.post(
    "/api/rank-groups/auto-assign",
    authenticateToken,
    async (req: any, res) => {
      res.status(403).json({
        success: false,
        message:
          "MARIANA BASE RULE: Bulk assignment of users to rank groups is permanently disabled for security.",
  ...
  app.get(
    "/api/admin/search-analytics/keywords",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
        const limit = parseInt(req.query.limit as string) || 50;
        const topKeywords =
          await searchAnalyticsService.getTopSearchedKeywords(limit);
  ...
  app.get(
    "/api/admin/search-analytics/keyword/:keyword",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
  ...
  app.get(
    "/api/admin/search-analytics/summary",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
        const summary =
          await searchAnalyticsService.getSearchAnalyticsSummary();
        res.json(summary);
      } catch (error) {
  ...
  app.post("/api/ai-feedback", authenticateToken, async (req: any, res) => {
    try {
      ...
      const isAdminUser =
        req.userId === "44885683" ||
        req.userId === "+919029010070" ||
        req.userId === "5791e66f-9cc1-4be4-bd4b-7fc1bd2e258e";
      ...
      if (
        ...
        !answerId ||
        !feedbackType ||
        !["helpful", "needs_improvement"].includes(feedbackType)
      ) {
        return res.status(400).json({
          success: false,
      ...
      console.log(
        `AI Feedback recorded: Q${questionId} A${answerId} - ${feedbackType} by ${adminId}`,
      );
  ...
  app.get(
    ...
    async (req: any, res) => {
      ...
      `,
        ...
        console.log(
          `Retrieved ${feedbackResult.rows.length} feedback records for admin ${adminId}`,
        );
      ...
      if (planType === "premium") {
        if (!billingPeriod || !["monthly", "yearly"].includes(billingPeriod)) {
          return res.status(400).json({
            success: false,
            message:
              "Invalid billing period for premium plan. Must be monthly or yearly",
          });
        }
      }
      ...
      if (planType === "super_user") {
        if (!topupPlan || !["topup_451", "topup_4510"].includes(topupPlan)) {
          return res.status(400).json({
            success: false,
            message:
              "Invalid topup plan for super user. Must be one of the available topup options",
          });
        }
      }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        const userId = req.userId;
        const subscriptions =
          await razorpayService.getUserSubscriptions(userId);
  ...
  app.post("/api/admin/activate-premium", async (req, res) => {
    try {
      ...
      console.log("🔧 Admin activating premium for:", {
        userId,
        email,
        subscriptionType,
        paymentId,
      ...
      const expiryInterval = subscriptionType.includes("yearly")
        ? "1 year"
        : "1 month";
  ...
  app.post(
    ...
    async (req: any, res) => {
      ...
      `,
        ...
        console.log(
          `✅ Admin linked payment to user: subscription ${subscriptionId} -> user ${userId}`,
        );
  ...
  app.post(
    "/api/admin/process-manual-payment",
    authenticateToken,
    isAdmin,
    async (req: any, res) => {
      try {
        ...
        console.log(
          `🔧 Manual payment processing: ${email} - Payment ID: ${paymentId}`,
        );
      ...
      `,
        ...
        console.log(
          `✅ Manual payment processed successfully: ${email} - Payment ID: ${paymentId}`,
        );
    ...
    try {
      ...
      `,
        [
          ...
          method,
          subscriptionType
            ? `Premium Payment - ${subscriptionType} ${billingPeriod}`
            : description,
          method === "upi" ? "upi" : "card",
          JSON.stringify({
            subscriptionType,
      ...
      `);
        ...
        res.json({
          ...
          pool: {
            ...
            errorRate:
              poolStats.queryCount > 0
                ? (
                    (poolStats.connectionErrors / poolStats.queryCount) *
                    100
                  ).toFixed(2) + "%"
                : "0%",
          },
      ...
      try {
        ...
        switch (action) {
          case "restart-keeper":
            ...
            res.json({
              message: "Database keeper restarted",
              action: "restart-keeper",
            });
            break;
          ...
          case "force-sync":
            ...
            res.json({
              message: "Synchronization forced",
              action: "force-sync",
            });
            break;
    ...
    async (req, res) => {
      try {
        ...
        if (!operation || !data) {
          return res
            .status(400)
            .json({ error: "Operation and data are required" });
        }
        ...
        if (data.type === "auth") {
          ...
          try {
            ...
            userInfo = data.userInfo || {
              ...
              maritimeRank: data.rank,
            };
            isAuthenticated = true;
            activeConnections.set(userId, ws);
            console.log(
              `User ${userId} authenticated and connected via WebSocket`,
            );
          ...
          if (!rank) {
            ws.send(
              JSON.stringify({ type: "error", message: "Rank is required" }),
            );
            return;
          }
          ...
          if (!messageText || !rank) {
            ws.send(
              JSON.stringify({
                type: "error",
                message: "Message and rank are required",
              }),
            );
            return;
          }
          ...
          console.log(
            `📤 Broadcasting rank message from ${userInfo?.fullName} to ${rank}`,
          );
          ...
          try {
            ...
            `,
              [
                rank,
                userId,
                userInfo?.fullName || "Maritime Professional",
                messageText,
              ],
            );
            ...
            console.log(
              `✅ Message broadcast to ${rankRoom?.size || 0} users in ${rank} room`,
            );
          } catch (error) {
...
function generateUserQuestions(
  ...
  if (isEngineer) {
    relevantCategories = maritimeQuestions.filter(
      (cat) =>
        cat.category === "Engine" ||
        cat.category === "Safety" ||
        cat.category === "Port Operations",
    );
  } else if (isOfficer) {
    relevantCategories = maritimeQuestions.filter(
      (cat) =>
