...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Initialize geospatial indexing for performance optimization
  try {
    // Create composite indexes on location columns for faster geospatial queries
    await pool.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_device_location 
      ON users (device_latitude, device_longitude) 
      WHERE device_latitude IS NOT NULL AND device_longitude IS NOT NULL
    `);
    
    await pool.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_current_location 
      ON users (current_latitude, current_longitude) 
      WHERE current_latitude IS NOT NULL AND current_longitude IS NOT NULL
    `);
    
    await pool.query(`
      CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_users_profile_location 
      ON users (latitude, longitude) 
      WHERE latitude IS NOT NULL AND longitude IS NOT NULL
    `);
    
    console.log('✅ Geospatial indexes created for optimized sailor search');
  } catch (indexError) {
    console.log('ℹ️  Geospatial indexes may already exist or creation failed:', (indexError as Error).message);
  }

  // OPTIMIZED GEOSPATIAL SAILOR SEARCH - Find nearby users within radius using PostgreSQL Haversine
  app.get("/api/discover/nearby", async (req, res) => {
    try {
      ...
      console.log(`🔍 Geospatial search: Finding sailors within ${searchRadius}km of (${userLat}, ${userLng})`);

      // PostgreSQL query with Haversine formula for accurate distance calculation
      // This uses the earth's radius (6371 km) and trigonometric functions for precision
      const query = `
        SELECT 
          id,
          full_name,
          ...
          (
            6371 * acos(
              cos(radians($1)) * 
              cos(radians(COALESCE(device_latitude, current_latitude, latitude))) * 
              cos(radians(COALESCE(device_longitude, current_longitude, longitude)) - radians($2)) + 
              sin(radians($1)) * 
              sin(radians(COALESCE(device_latitude, current_latitude, latitude)))
            )
          ) AS distance_km
        FROM users 
        WHERE 
          (device_latitude IS NOT NULL AND device_longitude IS NOT NULL) OR
          (current_latitude IS NOT NULL AND current_longitude IS NOT NULL) OR
      ...
      const result = await pool.query(query, [userLat, userLng, searchRadius, resultLimit]);
      
      // Format response data with distance information
      const nearbyUsers = result.rows.map(user => ({
        id: user.id,
        fullName: user.full_name || 'Maritime Professional',
        userType: user.user_type || 'sailor',
        rank: user.rank || user.maritime_rank || 'Crew',
        maritimeRank: user.maritime_rank || user.rank || 'Maritime Professional',
        shipName: user.current_ship_name || user.ship_name || 'Shore-based',
        imoNumber: user.imo_number,
        port: user.port || 'Unknown Port',
        visitWindow: user.visit_window,
        city: user.city || 'Unknown',
        country: user.country || 'Unknown',
        // Prioritize device location > current location > profile location
        latitude: user.device_latitude || user.current_latitude || user.latitude,
        longitude: user.device_longitude || user.current_longitude || user.longitude,
        ...
        distance: Math.round(parseFloat(user.distance_km) * 10) / 10, // Round to 1 decimal
        locationSource: user.location_source || 'profile',
        locationUpdatedAt: user.location_updated_at,
        isRecentLocation: user.location_updated_at && 
          new Date(user.location_updated_at).getTime() > Date.now() - 24 * 60 * 60 * 1000 // Within 24 hours
      }));

      console.log(`✅ Found ${nearbyUsers.length} sailors within ${searchRadius}km`);
      ...
      const responseData = {
        ...
        totalResults: nearbyUsers.length,
        query: {
          executedAt: new Date().toISOString(),
          searchType: 'geospatial_proximity'
        }
      };
      ...
      res.status(500).json({ message: "Failed to find nearby sailors" });
    }
  });
