...
export interface IStorage {
  ...
  // SEMM Postcards
  createSemmPostcard(data: any): Promise<any>;
  getSemmPostcards(semmCode: string, semmType: string): Promise<any[]>;
  updateSemmPostcard(id: string, updates: any): Promise<any>;
  deleteSemmPostcard(id: string, userId: string): Promise<void>;
  likeSemmPostcard(userId: string, postcardId: string): Promise<void>;
  unlikeSemmPostcard(userId: string, postcardId: string): Promise<void>;
  shareSemmPostcard(userId: string, postcardId: string, shareType: string): Promise<void>;
  getSemmPostcardLike(userId: string, postcardId: string): Promise<any>;
  updateSemmPostcardCounts(postcardId: string): Promise<void>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getSemmPostcards(semmCode: string, semmType: string): Promise<any[]> {
    try {
      ...
      const result = await pool.query(`
        ...
        WHERE sp.semm_code = $1 AND sp.semm_type = $2 AND sp.is_active = true
  ...
  async updateSemmPostcard(id: string, updates: any): Promise<any> {
    try {
      const setClauses = Object.keys(updates).map((key, index) => `${key} = $${index + 2}`).join(', ');
      const values = [id, ...Object.values(updates)];
      
      const result = await pool.query(`
        UPDATE semm_postcards SET ${setClauses}, updated_at = NOW()
        WHERE id = $1
        RETURNING *
      `, values);
      ...
      console.error('Error updating SEMM postcard:', error);
      return null;
    }
  }

  async deleteSemmPostcard(id: string, userId: string): Promise<void> {
    try {
      await pool.query(`
        UPDATE semm_postcards SET is_active = false
        WHERE id = $1 AND user_id = $2
      `, [id, userId]);
    } catch (error) {
      console.error('Error deleting SEMM postcard:', error);
    }
  }

  async likeSemmPostcard(userId: string, postcardId: string): Promise<void> {
    try {
      await pool.query(`
        INSERT INTO semm_postcard_likes (user_id, postcard_id)
        VALUES ($1, $2)
        ON CONFLICT (user_id, postcard_id) DO NOTHING
      `, [userId, postcardId]);
      
      await this.updateSemmPostcardCounts(postcardId);
    } catch (error) {
      console.error('Error liking SEMM postcard:', error);
    }
  }

  async unlikeSemmPostcard(userId: string, postcardId: string): Promise<void> {
    try {
      await pool.query(`
        DELETE FROM semm_postcard_likes
        WHERE user_id = $1 AND postcard_id = $2
      `, [userId, postcardId]);
      
      await this.updateSemmPostcardCounts(postcardId);
    } catch (error) {
      console.error('Error unliking SEMM postcard:', error);
    }
  }

  async shareSemmPostcard(userId: string, postcardId: string, shareType: string): Promise<void> {
    try {
      await pool.query(`
        INSERT INTO semm_postcard_shares (user_id, postcard_id, share_type)
        VALUES ($1, $2, $3)
      `, [userId, postcardId, shareType]);
      
      await this.updateSemmPostcardCounts(postcardId);
    } catch (error) {
      console.error('Error sharing SEMM postcard:', error);
    }
  }

  async getSemmPostcardLike(userId: string, postcardId: string): Promise<any> {
    try {
      const result = await pool.query(`
        SELECT * FROM semm_postcard_likes
        WHERE user_id = $1 AND postcard_id = $2
      `, [userId, postcardId]);
      
      return result.rows[0];
    } catch (error) {
      console.error('Error getting SEMM postcard like:', error);
      return null;
    }
  }

  async updateSemmPostcardCounts(postcardId: string): Promise<void> {
    try {
      const likesResult = await pool.query(`
        SELECT COUNT(*) as count FROM semm_postcard_likes WHERE postcard_id = $1
      `, [postcardId]);
      
      const sharesResult = await pool.query(`
        SELECT COUNT(*) as count FROM semm_postcard_shares WHERE postcard_id = $1
      `, [postcardId]);
      
      await pool.query(`
        UPDATE semm_postcards
        SET likes_count = $1, shares_count = $2
        WHERE id = $3
      `, [likesResult.rows[0].count, sharesResult.rows[0].count, postcardId]);
    } catch (error) {
      console.error('Error updating SEMM postcard counts:', error);
    }
  }
