...
- **Equipment Breadcrumb Implementation (August 17, 2025)**: Added complete equipment breadcrumb structure using authentic maritime equipment from user screenshots. Equipment codes follow format aa, ab, ac, ad, ae for Propulsion; ba, bb, bc, bd, be for Power Generation, etc. Removed heart/like buttons from SEMM cards, keeping only share icons. Fixed duplicate code display issue by cleaning equipment titles (e.g., "aa aa Main Engine" now shows as "Main Engine"). Updated breadcrumb navigation to show "Machine Tree > System > Equipment" format instead of "Home > System > Equipment".
